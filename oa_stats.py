...
def get_eof_cast(data,world,good_vars):
    standard_matrix = np.zeros((len(np.ravel(data[list(good_vars)[0]])),(len(good_vars))))
    for i,v in enumerate(good_vars):
        standard_matrix[:,i] = (np.ravel(data[v]-world.averages[v])/world.stdevs[v])
    pc1vals = np.dot(standard_matrix,world.get_eof(rank=1))
    pc2vals = np.dot(standard_matrix,world.get_eof(rank=2))

    return pc1vals,pc2vals
...
class Distribution(object):
    ...
    def power_spectrum(self, window = None, sample_rate = 1, nchunks = 1, overlap = 50, 
        ...
        if odd_bool: freq = nu[0:N/2+1]
        else: freq = np.abs(nu[0:N/2+1])
        ...
        power = np.zeros((overlap_chunks,(N/2)+1))
        ...
        return avg_power/np.sum(avg_power), freq, dof
    ...
    def autocorrelation_plot(self, max_lag = 10, figname = None, **kwargs):
        ...
        ax.axhline(y = 1/2.718, color = 'black', linewidth = 1)
        ax.grid(True)
...
class Dataset(object):
    ...
    def __init__(self, data = None, data_type = 'attributes', method = 'svd', nx = None, ny = None, 
                weights = None, standardize = False):
    ...
    def into_matrix(self, varlist = None):
        if isinstance(self.data, dict) and self.data_type == 'attributes':
            self.raw_matrix = np.zeros((len(varlist), len(self.data[list(self.data.keys())[0]])))
            #print('oastats',np.shape(self.raw_matrix))
            ...
            for gv in range(len(varlist)):
                #print('oastats:',gv,varlist[gv])
                self.raw_matrix[gv,:] = self.data[varlist[gv]]
                self.anom_matrix[gv,:] = self.data[varlist[gv]]-np.average(self.data[varlist[gv]])
                print (np.shape(np.average(self.data[varlist[gv]])))
                self.averages[varlist[gv]]=np.average(self.data[varlist[gv]])
            
                self.stdevs[varlist[gv]] = np.std(self.data[varlist[gv]])
    ...
    def calculate_eofs(self):
        if self.method == 'svd':
            ...
            if self.data_type == 'spatial':
                U, s, V = np.linalg.svd(self.indata, full_matrices = False)
...
#			return self.eigenvectors[...,rank-1]
    ...
    def plot_eofs(self, ranks=[1,2,3], figsize=(10,8), linewidths=2, pcoption='timeseries', colors='on', fig=None, ax=None, **kwargs):
        ...
        print (colors)
        if pcoption == 'off':
            fig, ax = plt.subplots(n_eofs, figsize = figsize)
            for ne in range(n_eofs):

                ax[ne].plot(self.get_eof(rank = ranks[ne]), color = colors[ne], linewidth = linewidths)
                ax[ne].set_title('EOF %d'%(ranks[ne]))
            for a in ax[:]:
                a.set_xticks(np.arange(len(self.eof_labels)))
                a.set_ylabel('Standard Anomaly',fontsize=12)
        ...
        # loop thru the eofs
            for ne in range(n_eofs):
                ...
                if pcoption is not 'off':
                    ...
                    elif pcoption == 'distribution':
                        hist,eg = np.histogram(self.get_pc(rank=ranks[ne]),bins=np.linspace(-5,5,50),density=True)
                        hist *= 100./hist.sum()
                        ax[ne,1].plot(eg[:-1]+np.diff(eg)[0]/2.,hist, color=colors[ne], linewidth=1)
                        ax[ne,1].set_title('PC %d: %.1f%% variance'%(ranks[ne], self.get_variance(rank=ranks[ne])))
                    else:
                        pass
